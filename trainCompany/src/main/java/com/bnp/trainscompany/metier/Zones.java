...
public class Zones {
    ...
    public  static  List<Rules> regles;
    ...
    public static ZoneToFrom getZoneWithCost(String stationFrom, String stationTo){
        ...
        for (int i = 0; i < zonesFrom.size(); i++) {
            for (int j = 0; j < zonesTo.size(); j++) {
                ...
                List<Rules> reglesZones = regles.stream().filter(z -> {
                    return (z.from == zoneToFrom.getZoneFrom() && z.to == zoneToFrom.getZoneTo());
                }).collect( Collectors.toList() );
                zoneToFrom.setPrice(Long.valueOf(reglesZones.get(0).cost));
                zoneToFroms.add(zoneToFrom);
            }
        }

        List<ZoneToFrom> finalZones = zoneToFroms.stream()
                .sorted(Comparator.comparingDouble(ZoneToFrom::getPrice))
                .collect(Collectors.toList());
    ...
    public static void initRules(){
        ...
        regles.add(new Rules(1,1 , Long.valueOf("240")));
        regles.add(new Rules(1,2 , Long.valueOf("240")));
        regles.add(new Rules(2,1 , Long.valueOf("240")));
        regles.add(new Rules(2,2 , Long.valueOf("240")));

        regles.add(new Rules(3,3 , Long.valueOf("200")));
        regles.add(new Rules(3,4 , Long.valueOf("200")));
        regles.add(new Rules(4,3 , Long.valueOf("200")));
        regles.add(new Rules(4,4 , Long.valueOf("200")));

        regles.add(new Rules(3,1 , Long.valueOf("280")));
        regles.add(new Rules(3,2 , Long.valueOf("280")));

        regles.add(new Rules(4,1 , Long.valueOf("300")));
        regles.add(new Rules(4,2 , Long.valueOf("300")));

        regles.add(new Rules(1,3 , Long.valueOf("280")));
        regles.add(new Rules(2,3 , Long.valueOf("280")));

        regles.add(new Rules(1,4 , Long.valueOf("300")));
        regles.add(new Rules(2,4 , Long.valueOf("300")));
    }
